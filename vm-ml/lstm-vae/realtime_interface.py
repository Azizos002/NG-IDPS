...
Sprint 4 - API d'inférence temps réel (Architecture Deep LSTM-VAE - 32 Features)
+ Moteur de corrélation hybride (Rate-Limiter Burst + Whitelist externe JSON + Auto-Blocage)
+ Bouclier Global Anti-DDoS Spoofing (Pré-filtre volumétrique)
"""
...
CHEMIN_MODELE = os.path.join(DOSSIER_MODELE, "deep_lstm_vae.keras")
...
def charger_artefacts():
    logger.info("Chargement du modèle, du scaler et du seuil calibré...")
    ...
    # Utilisation prioritaire du seuil optimal calibré (sinon repli sur le 3-sigma baseline)
    ...
    logger.info(f"Seuil d'anomalie actif = {seuil:.6f}")
...
def traiter_message(message: dict, modele, scaler, seuil, producer_alertes):
    ...
    # Objectif : Détecter les DDoS Spoofés sans faire exploser le ML
    # =================================================================
    historique_global_paquets.append(temps_actuel)
    # Nettoyage des vieux paquets (garde les 2 dernières secondes)
    ...
    if len(historique_global_paquets) > SEUIL_GLOBAL_FLOOD:
        # On ne déclenche l'alerte globale qu'une fois toutes les 30 secondes pour ne pas spammer
        if (temps_actuel - derniere_alerte_globale) > 30.0:
            ...
            producer_alertes.send(KAFKA_TOPIC_ALERTS, {
                "action": "ENABLE_DDOS_SHIELD",  # Commande spéciale pour le SOAR
                ...
                "mse": 99.99, # Valeur factice maximale pour allumer le dashboard
                "seuil": seuil
            })
            producer_alertes.flush()

        # COURT-CIRCUIT : On ne fatigue pas le Modèle ML avec ces paquets poubelles !
        return

    # =================================================================
    # 2. ANALYSE COMPORTEMENTALE IA (LSTM-VAE) ET IP
    # =================================================================
    features = extraire_features(message)
    if features is None:
        return

    # --- DÉTECTION DE BURST / SPAM DE PAQUETS (Rate-Limiting par IP) ---
    ...
    # --- GESTION DU BUFFER GLISSANT POUR LE LSTM-VAE ---
    ...
    mse = float(np.mean(np.square(sequence_norm - np.array(reconstruction))) * 1000.0)

    # --- LOGIQUE DE CORRÉLATION HYBRIDE ---
    anomalie_ml_stricte = mse > seuil
    ...
    logger.info(
        f"IP={src_ip} | MSE={mse:.4f} | paquets_2s={nb_paquets_recents} | "
        f"depassements_consecutifs={compteurs_depassement[src_ip]} | latence={latence_ms:.3f}ms"
    )
    ...
    if compteurs_depassement[src_ip] >= SEUIL_PERSISTANCE:
        if src_ip in WHITELIST_IPS:
            ...
            executer_auto_block(src_ip, mse, seuil, latence_ms, producer_alertes)

        compteurs_depassement[src_ip] = 0  # Reset après traitement
...
def main():
    ...
    logger.info("En attente de flux réseau en temps réel... (Ctrl+C pour arrêter)")

    try:
        for record in consumer:
            traiter_message(record.value, modele, scaler, seuil, producer_alertes)
    except KeyboardInterrupt:
        logger.info("Arrêt de l'interface temps réel demandé par l'utilisateur.")
    finally:
        consumer.close()
